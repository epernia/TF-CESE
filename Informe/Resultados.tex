...
El efecto que se consigue es que cuando \textit{TransferTo} retorna, no retorna al proceso que lo llamó, sino al siguiente proceso en el punto en que llamó a \textit{TransferTo} previamente. 
...
Si se  ejecuta esta aplicación en una Terminal Serie, se puede ver el siguiente partón de ejecución de las tareas manejadas por el planificador:
...
Los puntos suspensivos indican que se reciben múltiples lineas iguales y se indican las conmutaciones entre las tareas 1 y 2 hasta que termina la ejecución.
...
La aplicación SCJ está compuesta por un \textit{mission sequencer}, una \textit{mission} y un único \textit{handler} de eventos periódicos. Los \textit{handlers} son objetos planificables\footnote{En inglés \textit{schedulable objects}.}, planificados, en este caso, por el \textit{PriorityScheduler} SCJ Nivel 1.
