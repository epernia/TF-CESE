...
Un pin puede ser compartido por varios periféricos. Es por ello que se agrega al modelo el concepto de \textbf{Pin} de un periférico, que sirve para mantener la relación física del periférico con el microcontrolador. 
El Pin también sirve para determinar si un pin está en uso por un periférico cuando se intente emplear dos periféricos distintos que utilicen los mismos pines físicos.
...
Una clase \textbf{\textit{Peripheral}} define los métodos propuestos para el acceso, que luego son redefinidos en cada una de las subclases las cuales contienen la implementación concreta de cada tipo de periférico. Esta implementación incluye los métodos nativos que se deben implementar en C. En el método constructor del objeto Periférico se deben establecer un identificador de dispositivo constante (id) y los pines físicos que usará el dispositivo (también definidos como constantes).
...
\newpage
En la figura [\ref{fig:ModeloDevice}] se expone cómo interactúa un periférico con las otra clases.
Un \textbf{\textit{Device}} contiene una colección de pines y una de periféricos. Provee métodos para añadir tanto periféricos como pines, y consultar si está en uso un pin. Contiene métodos estáticos para la construcción de instancias para cada plataforma (EDU-CIAA-NXP y CIAA-NXP) con todos sus periféricos y pines construidos.
...
Con el diseño propuesto se debe mapear los pines de las plataformas. En este se deciden los números de pines, sus identificadores y se asignan las posibles funciones. En la figuras [\ref{fig:MapeoCiaa}] y [\ref{fig:MapeoEduCiaa}] se muestran los mapeos de pines de la CIAA-NXP y EDU-CIAA-NXP respectivamente.
...
Debido a que readaptar el Firmware de la CIAA era una tarea muy ardua se optó por crear una biblioteca en C que brinde el soporte necesario para los métodos nativos.
