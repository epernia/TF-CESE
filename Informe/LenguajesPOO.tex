...
Este lenguaje posee características modernas, entre ellas \textit{Garbage Collector}, que es un proceso que se encarga de detectar que objetos en memoria no se utilizan y los borra automáticamente, liberando al programador de esta tarea. Sin embargo, al igual que el caso anterior es una desventaja para aplicaciones de tiempo real pues la duración de su ejecución no es determinística. 
...
Posee comprobación estricta de tipos, como C++. Manejo de memoria automático mediante \textit{Garbage Collector} y utiliza referencias al igual que Python. Además, permite programación concurrente de forma estándar y existen varias especificaciones de Java para aplicaciones de tiempo real. 
...
La misma presenta un conjunto de clases Java que implementan soluciones \textit{Safety-Critical} para el inicio de la aplicación, concurrencia, planificación, sincronización, entrada/salida, gestión de memoria, gestión de temporización, procesamiento de interrupciones, interfaces nativas y excepciones.
...
Una entidad planificabl,e maneja una funcionalidad específica y tiene parámetros de liberación que describen el modelo de liberación y alcance temporal, por tiemplo tiempo de liberación y \textit{dedline}. 
...
\par\caption{Concepto de misión SCJ. Imagen de obtenida de [\ref{bib:PaperSCJKorsholm}].}\label{fig:missionsSCJ}
\end{center}
\end{figure}
...
\item 
\textbf{\textit{Memoria Inmortal.}} Es el área que perdura durante toda la vida útil del sistema quedando a cargo del programador.
\item 
...
\par\caption{Modelo de memoria SCJ. Imagen de obtenida de [\ref{bib:PaperSCJKorsholm}].}\label{fig:memoriaSCJ}
\end{center}
\end{figure}
