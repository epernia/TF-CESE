...
La salida del proceso de traducción de Java a C produce un conjunto de archivos fuente en lenguaje C. Algunos de estos archivos dependen de la aplicación a traducir y otros son fijos. Los archivos dependientes de la aplicación son:
...
\item
\textbf{natives\_XXX.c} - Funciones de HVM específicas de la arquitectura del microcontrolador.
\item
\textbf{XXX\_interrupt.s} - Funciones en \textit{assembler} para implementar un cambio de contexto si se utilizan hilos de ejecución (\textit{threading}).
\item
...
Para los programas no SCJ esto significa que los datos se asignan consecutivamente en áreas de memoria y se liberan fragmentos liberando un área completa a la vez.
...
\begin{itemize}
\item 
\textbf{Variables Nativas}. Sirven para mapear variables de C a variables estáticas de Clase en Java. Solo pueden ser accedidas dentro de métodos compilados.
\item 
